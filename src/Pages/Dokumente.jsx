...
export default function Dokumente() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const res = await fetch("http://localhost:5000/dokumente", {
    ...
    const handleView = (viewUrl) => {
        ...
        window.open(`http://localhost:5000${viewUrl}?token=${token}`, "_blank", "noopener,noreferrer");
    };
    ...
    const handleDownloadAll = async () => {
        try {
            ...
            const zip = new JSZip();
            for (const f of docs) {
                const res = await fetch(`http://localhost:5000${f.url}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                if (res.ok) zip.file(f.name, await res.blob());
            }
            saveAs(await zip.generateAsync({ type: "blob" }), "dokumente.zip");
        } catch (e) {
            console.error(e);
    ...
    return (
        ...
        <main className="section docs-section">
            <div className="docs-container">   {/* <- NEU, statt "container" */}
